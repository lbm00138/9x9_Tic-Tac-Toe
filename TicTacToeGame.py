...
def ent():
    ...
    pygame.draw.rect(screen, yeallow, (60 , 180, 240, 150))
    font = pygame.font.Font(None, 80)
    text = font.render("start", True, BLACK)
    text_rect = text.get_rect(center=(180, 255))
    screen.blit(text, text_rect)
    pygame.display.flip()
    font = pygame.font.Font(None, 80)
    text = font.render("OOXX", True, BLACK)
    text_rect = text.get_rect(center=(180, 55))
    screen.blit(text, text_rect)
    pygame.display.flip()
    font = pygame.font.Font(None, 60)
    text = font.render("O : X = {0} : {1}".format(O,X), True, BLACK)
    text_rect = text.get_rect(center=(180, 120))
    screen.blit(text, text_rect)
    pygame.display.flip()
    ...
    while True :
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                x, y = event.pos
                if x>=60 and x<=300 and y>=180 and y<=330:
                    start()
                    return ent()
